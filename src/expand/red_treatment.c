	...
	while (cmd)
	{
		...
		while (tmp_red)
		{
			if (tmp_red->type == HEREDOC)
				handle_heredoc(tmp_red->val, &cmd->input_fd);
			if (tmp_red->type == INPUT)
				handle_input(tmp_red->val, &cmd->input_fd);
			if (tmp_red->type == APPEND)
				handle_append(tmp_red->val, &cmd->output_fd);
			if (tmp_red->type == OUTPUT)
				handle_output(tmp_red->val, &cmd->output_fd);
